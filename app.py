...
def load_pbp(seasons: list[int]) -> pl.DataFrame:
    pl_df = nfl.load_pbp(seasons=seasons)
    cols = [
        "week",
        "posteam",
        "defteam",
...
if "pbp_key" not in st.session_state:
    st.session_state["pbp_key"] = None
if "last_position" not in st.session_state:
    st.session_state["last_position"] = position

run = st.button("Load or refresh data", type="primary")

pbp_key = (tuple(seasons), week_min, week_max)
if run or st.session_state["pbp"] is None or st.session_state["pbp_key"] != pbp_key:
    ...
    pbp["pressure_flag"] = (
        (col_or(pbp, "qb_hit") == 1) | (col_or(pbp, "sack") == 1) | (col_or(pbp, "pressure") == 1)
    ).astype(int)

    st.session_state["pbp"] = pbp
    st.session_state["pbp_key"] = pbp_key
...
pid_wr = pick(cols, ["receiver_player_id"])
pname_wr = pick(cols, ["receiver_player_name"])
pid_rb = pick(cols, ["rusher_player_id"])
pname_rb = pick(cols, ["rusher_player_name"])
pid_k = pick(cols, ["kicker_player_id"])
pname_k = pick(cols, ["kicker_player_name"])
team_off = pick(cols, ["posteam"])
team_def = pick(cols, ["defteam"])
...
def metric_catalog(pos: str):
    ...
    if "air_yards" in cols:
        yds.append(("Yardage • Air yards per attempt", lambda g: g.loc[g["pass_attempt"] == 1, "air_yards"].mean()))
    ...
    sit = [
        ...
        ("Situational • Pressure rate", lambda g: g["pressure_flag"].mean()),
    ]

    ball = [
        ("Ball security • Interception rate", lambda g: g.get("interception", pd.Series(index=g.index, dtype=float)).mean()),
        ("Ball security • Fumble rate", lambda g: g.get("fumble_lost", pd.Series(index=g.index, dtype=float)).mean()),
    ]
    ...
    if pos == "QB":
        ...
        vol = [
            ("Volume • Plays faced", lambda g: g["play_id"].count()),
            ("Volume • Dropbacks faced", lambda g: g["dropback"].sum()),
            ("Volume • Rush attempts faced", lambda g: g["rush_attempt"].sum()),
        ]
        yds = [
            ("Yardage • Yards allowed per play", lambda g: g["yards_gained"].mean()),
        ]
        sit = [
            ("Situational • Sack rate", lambda g: g.get("sack", pd.Series(index=g.index, dtype=float)).mean()),
            ("Situational • Pressure rate", lambda g: g["pressure_flag"].mean()),
        ]
        ball = [
            ("Ball security • Takeaway rate", lambda g: ((g.get("interception", 0) == 1) | (g.get("fumble_lost", 0) == 1)).mean()),
        ]
...
def aggregate_for_position(df: pd.DataFrame, pos: str):
    ...
    if t_col is None or t_col not in df.columns:
        st.error("This position needs a team column that is missing from the data. Click Load or refresh data again, or switch seasons.")
        st.stop()

    work = df.copy()
    work[t_col] = work[t_col].astype(str)
    work = work[work[t_col].notna() & (work[t_col] != "")]

    if name_col and name_col in work.columns:
        work = work[work[name_col].notna() & (work[name_col].astype(str) != "")]

    if pos == "Team Defense" or pos in ["Team Offense", "Team Overall"]:
        keys = [t_col]
    elif name_col and id_col and name_col in work.columns and id_col in work.columns:
    ...
    if pos in ["Team Offense", "Team Defense", "Team Overall"]:
        out["name"] = out[t_col]
...
priority = {
    ...
    "Team Defense": ["Efficiency • EPA allowed per play", "Efficiency • Success allowed", "Situational • Sack rate"],
...
if not ordered:
    st.error("No metrics available for this selection.")
    st.stop()

if position != st.session_state["last_position"]:
    st.session_state["x_metric"] = ordered[0]
    st.session_state["y_metric"] = ordered[1] if len(ordered) > 1 else ordered[0]
    st.session_state["last_position"] = position

with right:
    x_label = st.selectbox("X metric", ordered, key="x_metric")
    y_options = [m for m in ordered if m != x_label] or [x_label]
    y_label = st.selectbox("Y metric", y_options, key="y_metric")
...
count_cols = {
    ...
    "Volume • Plays",
    "Volume • Dropbacks",
    "Volume • Rush attempts",
    "Volume • Plays faced",
    "Volume • Dropbacks faced",
    "Volume • Rush attempts faced",
    "Kicking • FG attempts",
    "Kicking • FG made",
}
...
for _, r in agg.iterrows():
    ...
    if img is None:
        miss.add(abbr)
        ax.plot(xv, yv, marker="o", markersize=6)
    else:
        im = OffsetImage(img, zoom=float(logo_size))
...
ax.set_title(
    f"{position} scatter  {', '.join(map(str, seasons))}  weeks {week_min} to {week_max}  {min_label.lower()} {int(volume_threshold)}"
)
